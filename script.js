...
function setupEventListeners() {
    ...
    // Toggle Edit Mode
    elements.toggleEditModeBtn.addEventListener('click', toggleEditMode);
...
function renderMessage(text, type, time, imageUrl = null) {
    ...
    // Adicionar evento de clique para deletar ou editar
    msgDiv.addEventListener('click', (e) => {
        if (isDeleteMode) {
            ...
            // Lógica de edição
            const textSpan = msgDiv.querySelector('span:not(.material-icons):not(.msg-time)');
            if (textSpan) {
                const currentText = textSpan.innerText;
                const newText = prompt('Editar mensagem:', currentText);
                if (newText !== null) {
                    textSpan.innerHTML = newText.replace(/\n/g, '<br>');
                }
            } else if (imageUrl) {
                alert('Edição de texto não disponível para mensagens apenas com imagem.');
            }
        }
    });
...
function toggleEditMode() {
    ...
    if (isEditMode) {
        // Desativar modo exclusão se estiver ativo
        if (isDeleteMode) toggleDeleteMode();

        elements.toggleEditModeBtn.textContent = '✏️ Editar: ON';
        elements.toggleEditModeBtn.style.background = '#d97706'; // Laranja mais escuro
        elements.chatArea.classList.add('edit-mode');
    } else {
        elements.toggleEditModeBtn.textContent = '✏️ Editar: OFF';
        elements.toggleEditModeBtn.style.background = '#f59e0b'; // Laranja padrão
        elements.chatArea.classList.remove('edit-mode');
    }
}
